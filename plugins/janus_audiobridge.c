...
opus_int32 janus_audiobridge_buffer_energy(opus_int16 *_buffer, int _length, int *scale_factor);
opus_int16 WebRtcSpl_NormW32(opus_int32 a) {
	opus_int16 zeros;
...
opus_int32 janus_audiobridge_buffer_energy(opus_int16 *_buffer, int _length, int *scale_factor) {
    ...
    *scale_factor = WebRtcSpl_GetScalingSquare(_buffer, _length, _length);
    ...
    for (int i = 0, opus_int16 *p = _buffer; i < _length; i++) {
	...
	while(!g_atomic_int_get(&stopping) && audiobridge->destroyed == 0) {	/* FIXME We need a per-room watchdog as well */
		...
		for(ps = participants_list, participantCounter = 0; ps; participantCounter++) {
			...
			if(pkt != NULL) {
				...
				*/
				int scale_factor = 0;
				_curBufferStrength = janus_audiobridge_buffer_energy(curBuffer, samples, &scale_factor);
